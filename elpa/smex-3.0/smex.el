...
(defun smex-read-and-run (commands &optional initial-input)
  (let* ((chosen-item-name (smex-completing-read commands initial-input))
    ...
    (if smex-custom-action
      ...
      (unwind-protect
	  ;;; Note that this is old code that I have had to put in only because smex from elpa does not work with old emacs.
	  ;;; Basically I can't even update smex anymore.
	  (progn (setq prefix-arg current-prefix-arg)
            ...
            (run-at-time 0.01 nil (lambda (cmd) (setq last-repeatable-command cmd))
		     chosen-item)
        ;; NEW CODE that I had to comment (execute-extended-command current-prefix-arg chosen-item-name)
        (smex-rank chosen-item)))))
